...
df = pd.read_csv (r'C:\Users\ND.COM\VS Quantecon\lecture-julia.notebooks\pakistan_inflation1970.csv')   #read the csv file (put 'r' before the path string to address any special characters in the path, such as '\'). Don't forget to put the file name at the end of the path + ".csv"
print (df)
...
plt.title("Inflation in Pakistan")
...
inflation_data = model.sample(num_rows=205)
...
inflation_data = loaded.sample(num_rows=205)
...
plt.xlabel("Quarter")
plt.ylabel("Quarterly Inflation Rate (Annualized)")
plt.title("Synthetic Inflation Rate in Pakistan ")
