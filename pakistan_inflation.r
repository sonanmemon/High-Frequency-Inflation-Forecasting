...
data$quarter <- seq(from = as.Date("1970-01-01"), to = as.Date("2019-10-01"), by = 'quarter')
...
graph1 <- ggplot(data) +
  ...
  scale_x_date(breaks=date_breaks("4 year"), date_labels = "%Y-%m") +
  ...
  labs(x = "Quarter", y = "Quarterly Inflation Rate (Annualized)", title = "Inflation in Pakistan") +
...
monthlyinf <- ggplot(data_monthlyinf) +
  ...
  scale_x_date(breaks=date_breaks("1 month"), date_labels = "%Y-%m") +
  scale_y_continuous(breaks = c(0, 5, 10, 15, 20, 25, 30, 35, 40)) +
  labs(x = "Month", y = "Monthly Inflation Rate (Annualized)", title = "Recent Inflation in Pakistan") +
  theme(plot.title = element_text(hjust = 0.5), axis.text.x=element_text(angle=90))
